...
public final class GeoBiomeSource extends BiomeSource {
    ...
    // Minecraft multi-noise parameter lists for Overworld and Nether
    ...
    public GeoBiomeSource(long worldSeed, int dimensionId, HolderGetter<Biome> biomes) {
        ...
        this.kernel = new ScalarFieldKernel(worldSeed, this.profile);
    ...
    public Holder<Biome> getNoiseBiome(int quartX, int quartY, int quartZ, Climate.Sampler sampler) {
        ...
        // 1. The End: Center island (r < 1024) is The End; outer islands determined by erosion
        if (dimensionId == 2) {
            int chunkX = quartX >> 2;
            int chunkZ = quartZ >> 2;
        ...
        // 2. The Nether: Evaluated on the T-H plane (C=E=D=W=0) with offset=0
        if (dimensionId == 1) {
        ...
        // 3. The Overworld: 6-Dimensional Climate target point
    ...
    private Climate.TargetPoint convertToOverworldClimate(GeoSample sample, int worldY) {
        ...
        float erosion = computeErosion(sample.gradMagnitude, sample.riverIncision);
    ...
    private float computeContinentalness(double deltaH) {
        if (deltaH < -32.0) {
            ...
            double t = (deltaH - 4.0) / 16.0;
            return (float) (-0.11 + t * (0.03 - -0.11));
        } else if (deltaH <= 80.0) {
            double t = (deltaH - 20.0) / 60.0;
            return (float) (0.03 + t * (0.30 - 0.03));
        } else {
            double t = Math.clamp((deltaH - 80.0) / 300.0, 0.0, 1.0);
            return (float) (0.30 + t * 0.70);
        }
    }

    private float computeErosion(double slope, double incision) {
        double normSlope = Math.clamp(slope / 1.0, 0.0, 1.0);
        float baseErosion = (float) (0.75 - (1.75 * Math.pow(normSlope, 0.6)));
        if (incision > 4.0) {
            baseErosion = Math.clamp(baseErosion - 0.25f, -1.0f, 1.0f);
        }
    ...
    private float computeWeirdness(GeoSample sample) {
        ...
        if (sample.finalSurface > (double) (config.seaLevel() + 220)) return 0.0f;
